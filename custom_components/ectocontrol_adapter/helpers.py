"""Modbus client factory for ectoControl adapter."""
...
def create_modbus_client(config_data):
    """Create a Modbus client instance based on configuration data.
    ...
    if config_data[OPT_MODBUS_TYPE] == MODBUS_TYPE_TCP:
        return AsyncModbusTcpClient(
            host=config_data[OPT_HOST], port=int(config_data[OPT_PORT]), timeout=int(config_data[OPT_RESPONSE_TIMEOUT])
        )
